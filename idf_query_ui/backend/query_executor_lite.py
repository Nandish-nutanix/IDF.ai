...
SSH_PASSWORD = os.getenv("CVM_SSH_PASSWORD", "nutanix/4u")
...
def execute_python_query(
    ...
    `password` overrides the module-level SSH_PASSWORD (used to support PE
    clusters which authenticate with a different credential than PC clusters).
    """
    t_start = time.perf_counter()
    ssh_password = password or SSH_PASSWORD
...
def execute_query(
    ...
    if language in ("python", "py"):
        return execute_python_query(code, idf_ip, idf_port, timeout, password=password)
